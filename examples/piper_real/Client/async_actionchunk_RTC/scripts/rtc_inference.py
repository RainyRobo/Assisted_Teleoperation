...
class PI0:
    ...
    def __init__(self, host: str = "192.168.2.250", port: int = 8000, port_rtc: int = 8001, img_size: Tuple[int, int] = (224, 224)):
        ...
        self.policy_rtc = websocket_client_policy.WebsocketClientPolicy(host=host, port=port_rtc)
    ...
    def get_action(self) -> np.ndarray:
        ...
        try:
            ...
            end_act = time.time()
            print(f"Action inference time: {(end_act - start_act):.4f} seconds")
    ...
    def get_realtime_action(self, prefix_actions, inference_delay, prefix_attention_horizon, max_guidance_weight) -> np.ndarray:
        """Get real-time actions from the policy server.

        Returns:
            np.ndarray: Real-time actions.

        Raises:
            ValueError: If action shape is invalid.
        """
        print("AAAAAAAAAA: start to get realtime action")
        try:
            inputs = {
            ...
            print("AAAAAAAAAAAAAAAAA: start send inference:")
            start_act = time.time()
            # actions = self.policy_rtc.infer(inputs)["actions"]
            res = self.policy_rtc.infer(inputs)
            print("AAAAAAAAA: res", res.keys())
            actions = res["actions"]
            end_act = time.time()
            print(f"Realtime action inference time: {(end_act - start_act):.4f} seconds")
            if actions.shape != (50, 14):
                raise ValueError(f"Expected action shape (50, 14), got {actions.shape}")
            return actions
        except Exception as e:
            rospy.logerr(f"Failed to get real-time actions: {e}")
            raise
...
def save_action_chunk_plots(
    ...
    """
    为每个关节生成一张对比图，并保存到 save_dir：
      - prev_, actions, rtc_actions 三条曲线
      - infer_delay 处的垂直虚线
      - 标题为 “Joint {i}”
      - 文件名 joint_00.png ... joint_13.png
    """
    # 确保保存目录存在
    os.makedirs(save_dir, exist_ok=True)

    # 时间轴
    t_prev    = np.arange(prev_.shape[0])
    t_rtc     = np.arange(infer_delay,
                          infer_delay + rtc_actions.shape[0])

    for i in range(14):
        plt.figure(figsize=(8, 4))
        # 历史动作
        plt.plot(t_prev,             prev_[:, i],
                 label='Prev',  alpha=0.3)
        # 实时推理
        plt.plot(t_rtc,            rtc_actions[:, i],
                 label='RTC',   alpha=0.7)
        # 推理延迟标记
        plt.axvline(x=infer_delay,
                    color='k',
                    linestyle='--',
                    linewidth=1,
                    label='Infer Delay' if i==0 else None)
        # 标题里写关节序号
        plt.title(f'Joint {i}')
        ...
        # 保存并关闭
        filename = os.path.join(save_dir, f'joint_{i:02d}.png')
        plt.savefig(filename)
        plt.close()


import concurrent.futures
plot_pool = concurrent.futures.ThreadPoolExecutor(max_workers=1)
...
class InferenceNode:
    ...
    def __init__(self):
        ...
        self.inference_loop = rospy.get_param("~inference_loop", 15)
        ...
        self.pub_trajectory = rospy.Publisher('/predicted_trajectory', Float64MultiArray, queue_size=1)
    ...
    def joint_left_callback(self, msg: JointState) -> None:
        ...
        # print("############### UPDATE LEFT STATE", time.time());
        self.state_timestamp=time.time();
    ...
    def all_data_ready(self) -> bool:
        ...
        print("AAAAAAAAAAAAAAAAAAA:", missing);
        if missing:
            rospy.logwarn(f"Missing data: {', '.join(missing)}")
            return False
        print("NONONNO:", missing);
        return True
    ...
    def run(self, model: PI0) -> None:
        """Run the inference loop, processing observations and publishing actions with detailed timing."""
        RATE_HZ = 3
        rate = rospy.Rate(RATE_HZ)
        seq = 0
        frm_idx = 0

        # Initialize previous actions with zeros
        prev_actions = np.zeros((50, 14))
        delay_step = 0
        ...
        while not rospy.is_shutdown():
            ...
            if self.all_data_ready() and self.update_state():
                try:
                    ...
                    if delay_step == 0:
                        prefix = np.zeros((50, 14))
                    else: 
                        # prefix = np.concatenate([
                        #     prev_actions[delay_step:] - prev_actions[delay_step-1:-1],
                        #     np.zeros((delay_step, 14)),
                        # ], axis=0)
                        prefix = np.concatenate([
                            prev_actions[delay_step:],
                            np.zeros((delay_step, 14)),
                        ], axis=0)

                    # Run both inferences concurrently
                    # print("AAAAAAAAAAAAA prefix: ", prefix[:, 0])
                    # prefix = {"actions": prefix, "state": [0] * 14}
                    remts = time.time()
                    actions = model.get_realtime_action(None, delay_step, 50 - delay_step, 10)
                    # print(f"AAAAAAAAAAAAA: {actions[:, 0]}, shape: {actions.shape}")
                    t_inference = time.time() - remts
                    print("Inference time:", t_inference)

                    # Publish trajectory
                    flat = np.concatenate([actions.flatten(), np.array([remts])], axis=0)
                    msg = Float64MultiArray(data=flat.tolist())
                    ...
                    # Compute prefix attention horizon based on loop duration
                    loop_end = time.time()
                    loop_duration = loop_end - loop_start
                
                    rate.sleep()
                    print("prefix action horizon:", 50 - delay_step, "delay step:", delay_step)
                    print(f"loop duration: {loop_duration:.4f}, sleep time: {(time.time() - loop_end):.4f}")
                except Exception as e:
...
def main():
    ...
    try:
        ...
        node = InferenceNode() 
        print("AAAAAAAAAAAAA: Node initialized.")
        node.run(model)
        # asyncio.run(node.run(model))
