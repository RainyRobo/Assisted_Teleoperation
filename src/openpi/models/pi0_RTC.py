...
class Pi0RTCConfig(_model.BaseModelConfig):
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0_RTC":
        return Pi0_RTC(self, rngs=nnx.Rngs(rng))
...
class Pi0_RTC(_model.BaseModel):
        ...
        *,
        ...
        def step(carry):
            ...
            def pinv_corrected_velocity(observation, x_t, y, t):
                idx = jax.lax.axis_index("batch")          # vmap 给每个样本的“逻辑 id”
                prefix_mask_i   = prefix_mask[idx]         # (P,)      ← 把(B,P)切成(P,)
                prefix_tokens_i = prefix_tokens[idx]       # (P,)
                kv_cache_i      = jax.tree_map(lambda v: v[idx], kv_cache)
                def denoiser(x_t):
                    ...
                    # prefix tokens
                    prefix_attn_mask = einops.repeat(prefix_mask_i, "b p -> b s p", s=suffix_tokens.shape[1])
                    ...
                    assert full_attn_mask.shape == (
                        ...
                        prefix_tokens_i.shape[1] + suffix_tokens.shape[1],
                    )
                    # `positions` is shape (b, suffix_len) indicating the positions of the suffix tokens
                    positions = jnp.sum(prefix_mask_i, axis=-1)[:, None] + jnp.cumsum(suffix_mask, axis=-1) - 1

                    (prefix_out, suffix_out), _ = self.PaliGemma.llm(
                        [None, suffix_tokens], mask=full_attn_mask, positions=positions, kv_cache=kv_cache_i
                    )
            ...
            v_t = pinv_corrected_velocity(
                observation, x_t, prev_action_chunk, time
            )
